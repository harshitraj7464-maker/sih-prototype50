...
map_view = st.sidebar.radio(
    ...
    ["Topographic Roads (Default)", "High-Resolution Terrain Imagery (Global Node)"]
)
...
with col_map:
    ...
    # FIX: Using open-source terrain tiles built into Folium that never hit server proxy blocks
    if map_view == "High-Resolution Terrain Imagery (Global Node)":
        m = folium.Map(
